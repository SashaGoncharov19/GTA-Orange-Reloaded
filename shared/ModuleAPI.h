// Shared interface between the GTA:Orange server and its native modules
// (lua-module, simple-module, ...).
//
// IMPORTANT: this header defines the binary contract (vtable layout of APIBase
// and the layout of MValue) between orange_server and every module DLL/.so.
// Both sides must be compiled from the very same version of this file.
...
// Symbol export helper for module entry points (Validate, OnModuleInit, ...)
#if defined(_WIN32)
#define ORANGE_EXPORT __declspec(dllexport)
#else
#define ORANGE_EXPORT __attribute__((visibility("default")))
#endif
